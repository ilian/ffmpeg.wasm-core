...
FLAGS=(
  ...
  -s EXIT_RUNTIME=0                             # do not exit runtime after execution
  -s MODULARIZE=1                               # use modularized version to be more flexible
  -s EXPORT_NAME="createFFmpegCore"             # assign export name for browser
  -s EXPORTED_FUNCTIONS="[_main]"  # export main and proxy_main funcs
  -s EXPORTED_RUNTIME_METHODS="[FS, FS_mount, FS_unmount, FS_filesystems, callMain, cwrap, ccall, setValue, writeAsciiToMemory, stringToUTF8, lengthBytesUTF8]"   # export preamble funcs
